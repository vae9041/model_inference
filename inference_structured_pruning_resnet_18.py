...
Designed for checkpoints produced by `structured_pruning_resnet_18.py` (uses
`load_structured_pruned_state_dict` when `structured_pruning_meta` is present).

Input layout matches training / eval: list of float CHW tensors in [0, 1], resized to --width x --height.

Examples:
  python inference_structured_pruning_resnet_18.py \\
    --checkpoint ./models_resnet18_v3_structured/structured_pruned.pth \\
    --image_dir ./my_images

  python inference_structured_pruning_resnet_18.py \\
    --checkpoint ./models_resnet18_v3_structured/structured_pruned.pth \\
    --image_path ./my_images/test.png \\
...
def load_model(checkpoint_path: Path, device: torch.device) -> torch.nn.Module:
    ...
    if isinstance(ckpt, dict) and "model_state_dict" in ckpt:
        if ckpt.get("structured_pruning_meta"):
            load_structured_pruned_state_dict(model, ckpt, device)
            logger.info("Loaded structured-pruned checkpoint (wrapper + shapes)")
        else:
            model.load_state_dict(ckpt["model_state_dict"])
            logger.info("Loaded plain model_state_dict")
    else:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Inference + latency benchmark for pruned ResNet-18 detector")
    p.add_argument("--checkpoint", type=str, required=True, help="Path to structured_pruned.pth or compatible .pth")
    ...
    for path in paths:
        ...
        if save_dir is not None:
            # Boxes are in resized (width x height) space; draw on the same resolution.
            rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
