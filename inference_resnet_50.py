#!/usr/bin/env python3
"""
Run Faster R-CNN (ResNet-50 FPN) inference on images from a file or directory and benchmark latency.

Matches training in `train.py`: `fasterrcnn_resnet50_fpn` + `FastRCNNPredictor` for 2 classes
(background + grasp). Loads `model_state_dict` from checkpoints such as `final_model.pth`.
...
def get_model(num_classes: int = 2, freeze_backbone: bool = False) -> nn.Module:
    """Same architecture as `train.get_model` for ResNet-50 FPN (no pretrained download)."""
    try:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Inference + latency benchmark for ResNet-50 Faster R-CNN")
    p.add_argument(
        "--checkpoint",
        type=str,
        default="./model_resnet_50/final_model.pth",
        help="Path to .pth (e.g. model_resnet_50/final_model.pth)",
    )
    ...
    logger.info("Architecture: Faster R-CNN, ResNet-50 FPN backbone (num_classes=2)")
